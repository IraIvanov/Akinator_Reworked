...
int game ( node_t* node, list_t* seek_list ){ 
    ...
    while ( tree ) {        
        ...
        if ( !stricmp( string, "N") ) {
            ...
            if ( !tree->right ) {
                ...
                scanf( "%[^\n]", string);
                int i = 0;
                i = tree_seek( node, node->node, string, seek_list, 0);
...
int show_def( const node_t* node, list_t* seek_list ) {
...
int show_tree( const node_t* node ) {
...
int compare( const node_t* node, list_t* seek_list1, list_t* seek_list2 ) {
    ...
    i = tree_seek( node, node->node, string, seek_list1, 0);
    ...
    while(getchar() != '\n' ){};
    scanf( "%[^\n]", cmp_string);
    j = tree_seek( node, node->node, cmp_string, seek_list2, 0);
